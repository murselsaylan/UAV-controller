...
def simulasyon_adimi():
    ...
    for iha in data:
        yeni_iha = dict(iha)
        if iha["id"].startswith("kamik"):
            # Kamikaze sadece ilk waypoint'e gider, ulaştıysa sabit kalır
            wp_iha = iha_waypoint_list_gotur(yeni_iha, step=5, havas=havas)
            if wp_iha is not None:
                yeni_iha = wp_iha
            if "wp_index" in yeni_iha and yeni_iha["wp_index"] > 0:
                yeni_iha["wp_index"] = 0
            if "z" in yeni_iha:
                yeni_iha["z"] = yukari_asagi(yeni_iha["z"])
        elif iha["id"].startswith("ilger") or iha["id"].startswith("Hedef"):
            wp_iha = iha_waypoint_list_gotur(yeni_iha, step=5, havas=havas)
            if wp_iha is not None:
                yeni_iha = wp_iha
            if "z" in yeni_iha:
